...
const RegisterForm = ({ returnUrl }: RegisterFormProps) => {
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <div className="space-y-2">
        <label className="atelier-label" htmlFor="name">
          昵称（可选）
        </label>
        <input
          autoComplete="name"
      ...
      <div className="space-y-2">
        <label className="atelier-label" htmlFor="confirmPassword">
          确认密码
        </label>
        <input
          autoComplete="new-password"
      ...
      {state.success ? (
        <p className="atelier-alert-success" role="status" aria-live="polite">
          注册成功，正在登录…
        </p>
      ) : null}
      <button
