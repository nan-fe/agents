...
const poolAction =
  (messageComppnent: ReturnType<typeof App.useApp>['message']) =>
  async (prevState: PoolState, action: PoolAction): Promise<PoolState> => {
  if (action.type === 'load') {
    try {
      ...
      await reportError(error, 'product-pool/load');
      messageComppnent.error('加载选品池失败，请稍后重试');
  ...
  if (action.type === 'preview') {
    try {
      ...
      messageComppnent.success('商品信息识别完成');
      return {
        ...prevState,
        drawerStep: 'preview',
      ...
      await reportError(error, 'product-pool/preview', { url: action.url });
      messageComppnent.error(error instanceof Error ? error.message : '识别商品失败，请稍后重试');
      return prevState;
    }
  }
  ...
  if (action.type === 'confirm') {
    try {
      ...
      messageComppnent.success(response.message || '商品已加入选品池');
      const items = await fetchProductItems();
      return resetDrawerState({ ...prevState, items });
    } catch (error) {
      console.error('添加商品失败:', error);
      await reportError(error, 'product-pool/confirm');
      messageComppnent.error(error instanceof Error ? error.message : '添加商品失败，请稍后重试');
      return prevState;
    }
  }
  ...
  if (action.type === 'delete') {
    try {
      ...
      messageComppnent.success('已删除');
      const items = await fetchProductItems();
      return { ...prevState, items };
    } catch (error) {
      console.error('删除商品失败:', error);
      await reportError(error, 'product-pool/delete', { productId: action.productId });
      messageComppnent.error('删除失败，请稍后重试');
      return prevState;
    }
  }
  ...
  if (action.type === 'open-detail') {
    try {
      ...
      await reportError(error, 'product-pool/open-detail', { productId: action.productId });
      messageComppnent.error(error instanceof Error ? error.message : '加载商品详情失败');
...
const ProductPool = () => {
  ...
  const [poolState, dispatch, isPending] = useActionState(poolAction(message), initialPoolState);
