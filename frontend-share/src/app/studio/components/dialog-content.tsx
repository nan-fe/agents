...
import {
  ...
  import {
    ...
    type StreamMessage,
  } from '@/lib/sse/sse-resume';
  import { createStreamIngestor } from '@/lib/sse/sse-stream-ingest';
    ...
    try {
      const fallbackResult = await deps.connect<StreamEvent, DialogResultData>({
        ...
        fallback: async (error, lastEventId) => {
          ...
          messageApi.warning('实时通道异常，正在切换降级模式…');
      ...
      await reportError(error, 'dialog/generateContent', { prompt: payload.prompt });
      return {
        kind: 'error',
  ...
  const executeDialogGeneration = async (
    ...
    try {
      ...
      if (outcome.kind === 'success') {
        ...
        payload.messageApi.error(outcome.error);
      }
    } finally {
      onSettled();
    }
    ...
    const startGeneration = (prompt: string) => {
      ...
      void (async () => {
        ...
        try {
          ...
          await reportError(error, 'dialog/createProject');
        ...
        const activeEpoch = generationEpochRef.current;
        setGenerationEpoch(activeEpoch);
        ...
        void executeDialogGeneration(
          ...
          () => {
            if (generationEpochRef.current !== activeEpoch) {
              return;
            }
    ...
    const handleStopGeneration = () => {
      modal.confirm({
        ...
        onOk: () => {
          ...
          disconnect();
          setIsGenerating(false);
          setThread((prev) => prev.filter((item) => item.type !== 'pending'));
                ...
                >
                  {versionOptions.map((version) => (
                    <Select.Option key={version.id} value={version.id}>
                      V{version.id + 1} · {formatTimestamp(version.timestamp)}
