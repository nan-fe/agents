...
### Changed 2026-07-10 — 微博 Profile 登录与社交媒体热点分析

- **微博登录（Profile 浏览器）**
  - 创作台点击「登录微博」会打开本机真实浏览器窗口，在窗口内完成账号登录或扫码
  - 登录完成后点击「我已登录」，系统检测登录态并持久化 Profile，后续发博无需重复登录
  - 登录确认记录写入数据库，减少每次查看发布设置时重复启动浏览器检测
  - 移除 Studio 内截图点按式登录，交互更简单、更接近真实浏览器体验

- **社交媒体热点分析**
  - 创作台侧边栏新增「热点分析」，一键生成商品宣传相关多平台热点速报
  - 覆盖微博、小红书、抖音、X（Twitter）、Reddit 五个平台，按平台分栏展示热点条目
  - 流式返回分析进度：各平台热点逐条推送，并汇总跨平台共性话题与营销洞察建议
  - 支持刷新重新分析；相同关键词与平台组合在缓存有效期内可秒开历史结果
