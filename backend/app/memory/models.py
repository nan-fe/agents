...
class WeiboAuthRow(Base):
    """微博 Playwright Profile 登录确认记录（按 Studio user_id）。"""
...
class WeiboUserTokenRow(Base):
    """用户微博 OAuth 2.0 授权后的 access_token。"""

    __tablename__ = "weibo_user_tokens"

    user_id: Mapped[str] = mapped_column(String(128), primary_key=True)
    access_token: Mapped[str] = mapped_column(Text, nullable=False)
    expires_at: Mapped[datetime] = mapped_column(_utc_datetime, nullable=False)
    weibo_uid: Mapped[str | None] = mapped_column(String(64), nullable=True)
    weibo_screen_name: Mapped[str | None] = mapped_column(String(128), nullable=True)
    scope: Mapped[str | None] = mapped_column(String(512), nullable=True)
    updated_at: Mapped[datetime] = mapped_column(_utc_datetime, nullable=False, default=_utcnow)
