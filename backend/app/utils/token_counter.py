...
class TokenCounter:
    ...
    def count_messages(self, messages: List[Dict[str, Any]], model_name: str = "default") -> int:
        """计算消息列表的token数
    ...
    def calculate_available_space(self, history_tokens: int, current_question_tokens: int, 
                               model_name: str = "default", 
                               answer_reserve: Optional[int] = None) -> Tuple[int, int, int, int, bool]:
        """计算可用的上下文空间
        ...
        reserve = answer_reserve if answer_reserve is not None else self.default_answer_reserve
    ...
    def format_context_analysis(self, history_tokens: int, current_question_tokens: int, 
                              model_name: str = "default", 
                              answer_reserve: Optional[int] = None) -> str:
        """格式化上下文分析结果
        ...
        context_window, used_space, reserve, available_space, is_sufficient = \
            self.calculate_available_space(history_tokens, current_question_tokens, model_name, answer_reserve)
...
"""
    ...
    def calculate_max_tokens(self, input_tokens: int, model_name: str = "default", 
                          safety_buffer: int = 200) -> int:
        """计算最大输出token数
    ...
    def calculate_input_tokens(self, prompt: str, history: str = "", model_name: str = "default") -> int:
        """计算输入的总token数
